...
function isLoopRoute(route: ScoredRoute): route is LoopRoute {
  return 'floorability' in route
}
...
export default function RouteCard({
  ...
  const loop = isLoop && isLoopRoute(route) ? route : null
    ...
    return (
      <button
        ...
        style={{
          ...
          border: isSelected ? `2px solid ${route.color}` : '2px solid transparent',
          boxShadow: isSelected ? `0 0 20px ${route.color}15` : '0 1px 6px rgba(0,0,0,0.2)',
        }}
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            {loop && <FloorabilityBadge floorability={loop.floorability} compact />}
        ...
        <div className="flex items-center gap-3 mt-1.5 pl-[18px]">
          ...
          {loop && (
            <span className="text-[11px] text-[#8a8aa0]">
              {loop.floorability.floorItCount} floor-it events
            </span>
          )}
          {!loop && route.highlights[0] && (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-white font-bold text-base tracking-tight">{route.name}</h3>
        </div>
        {route.isFastest && !isLoop && (
        ...
        {isLoop && loop && (
          <span className="text-[10px] uppercase tracking-widest font-bold px-2 py-0.5 rounded-full"
            style={{
              color: loop.floorability.totalScore >= 50 ? '#ff2d55' : '#6a6a8a',
              backgroundColor: loop.floorability.totalScore >= 50 ? '#ff2d5515' : '#6a6a8a15',
            }}>
            {loop.loopStyle}
          </span>
        )}
      ...
      <div className="flex items-baseline gap-3 mb-3">
        ...
        {isLoop && <span className="text-sm text-[#4a4a5a]">loop</span>}
      </div>

      {/* Delta time (A→B mode) */}
      {!isLoop && !route.isFastest && route.deltaMin > 0 && (
        <div className="mb-3">
          <span
            ...
            style={{
              ...
              backgroundColor: route.deltaMin <= 5 ? '#22c55e12' : route.deltaMin <= 15 ? '#ffb80012' : '#ff2d5512',
            }}
          >
      ...
      {/* Floorability Score (Loop mode) */}
      {loop && (
        <div className="mb-3">
          <FloorabilityBadge floorability={loop.floorability} />
        </div>
      )}
      ...
      {route.highlights.length > 0 && (
        <div className="space-y-1">
          {route.highlights.map((h, i) => (
            <div key={i} className="text-xs text-[#8a8aa0] flex items-center gap-1.5">
